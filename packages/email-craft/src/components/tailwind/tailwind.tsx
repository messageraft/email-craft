...
const debugProps = debug.elements.enabled ? { dataType: 'email-craft/tailwind' } : {};
...
const getUno = (config: ConfigBase, production: boolean) => {
  ...
  if ((config?.theme as any)?.extend) {
    warn(
      'email-craft → Tailwind: Use of `theme.extend` is not necessary. `theme.extend` has been merged into `theme`'
    );
