...
const Renderer = (props: ConditionalProps) => {
  ...
  return <email-craft-cond dangerouslySetInnerHTML={{ __html: innerHtml }} />;
};
...
export const Conditional: JsxEmailComponent<ConditionalProps> = (props) => {
  ...
  if (typeof expression === 'undefined' && typeof mso === 'undefined')
    throw new RangeError(
      'email-craft: Conditional expects the `expression` or `mso` prop to be defined'
    );

  if (typeof expression !== 'undefined' && typeof mso !== 'undefined')
    throw new RangeError(
      'email-craft: Conditional expects the `expression` or `mso` prop to be defined, not both'
    );
